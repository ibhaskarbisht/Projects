...
//        l5.setIcon(img);
        ...
        Font F7=new Font("aril",Font.BOLD,20); 
        l7.setFont(F7);
        l5.add(l7);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b1){
            try{
                ...
                String str2 = "select * from employee where emp_id ='" + t.getText()+"'";
                
                ResultSet rs = con.s.executeQuery(str2);
                
                if(rs.next())
                {
                	  String locationToBeDeleted = rs.getString(5);
                	  String str3 = "select * from ongc where location='"+locationToBeDeleted+"'";
                	  ResultSet rs2 = con.s.executeQuery(str3);
                	  
                	  if(rs2.next())
                	  {
                		  int prevCount = rs2.getInt(2);
                		  int newCount = prevCount - 1;
                		  
                		  if(newCount<0)
                			  newCount = 0;
 String str4 = "update ongc set count = '" + newCount + "'" + "where location = '" +locationToBeDeleted + "'";
